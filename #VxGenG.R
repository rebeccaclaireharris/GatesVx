...
#   }
#   
  # in elderly model is mass only in first 3 years after yrintro (then becomes continuous of a particular age, see later code for that )
...
#     }
    ...
    # If a masscampyear (start of them) then get 61-64yos (was 56-64 when vacc att 55, but reduced number of yrs to align with ages think wuold vac. Campaign over 3 yrs, so divide adult coverage by the 3 yrs of the campaign
    if (any(masscampyr==i)){
    ...
    # If a masscampyear (start of then) then get 16-19yos. Campaign over 3 yrs, so divide adult coverage by the 3 yrs of the campaign
    if (any(masscampyr==i)){
      thetaV4m[i,17:20]<-coverage/3
    }
